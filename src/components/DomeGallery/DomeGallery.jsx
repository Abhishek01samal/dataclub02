...
const DEFAULTS = {
  ...
  segments: 24 // Reduced from 35 for better base performance
};
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (!draggingRef.current && !inertiaRAF.current && !openingRef.current) {
        rotationRef.current.y = wrapAngleSigned(rotationRef.current.y + 0.12); // slow elegant spin
  ...
  // Gesture Implementation using native pointer events
  useEffect(() => {
    ...
    const onPointerMove = (e) => {
      ...
      if (!movedRef.current) {
        if (dxTotal * dxTotal + dyTotal * dyTotal > 16) {
  ...
  const openItemFromElement = useCallback(
    el => {
      ...
      if (el.classList.contains('item')) {
        imgContainer = el.querySelector('.item__image');
      } else if (el.classList.contains('item__image')) {
        item = el.parentElement;
      }

      if (!imgContainer || !item) return;

      openingRef.current = true;
      openStartedAtRef.current = performance.now();
      lockScroll();
      
      const elToFocus = imgContainer;
      focusedElRef.current = elToFocus;
      elToFocus.setAttribute('data-focused', 'true');
      
      const offsetX = getDataNumber(item, 'offsetX', 0);
      const offsetY = getDataNumber(item, 'offsetY', 0);
      const sizeX = getDataNumber(item, 'sizeX', 2);
      const sizeY = getDataNumber(item, 'sizeY', 2);
